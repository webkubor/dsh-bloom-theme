...
/* 表格：只强化分隔线，不加外框、不填表头底色。
   原因是 DSH 的单元格 padding 是 10px 16px 10px 0 —— 左内边距刻意为 0，
   好让表格首列与正文左对齐。此时若给表头填背景或给表格加外框，
   文字就会紧贴色块/边框边缘，看起来像边距坏了。
   另外圆角和 overflow:hidden 也不能加在 table 上：真正的横向滚动容器是外层的
   _tableScroll_（实测 scrollWidth 908 > clientWidth 748，表格本来就要滚），
   在 table 上裁剪会跟它打架，圆角还会随内容一起滚动。 */
th, td { border-color: var(--bloom-hairline) !important; }
thead th {
  border-bottom: 1px solid var(--bloom-hairline-strong) !important;
  font-weight: 600;
}
tbody tr:hover { background: rgba(var(--bloom-morandi), 0.06); }
...
/* ⚠️ 降权只能用「不影响布局」的属性。
   曾用 border-left + padding-left + margin-left 画左竖线：思考块是一串平铺的兄弟
   元素（p/ul/ol/h3/div），各自默认 margin 不同 —— p 的 margin-left 覆盖掉设定值，
   实测 p 在 x=626、ul 在 x=628，竖线错开 2px 对不成一条直线；ul 本身还有列表缩进，
   再加 12px padding 会把编号挤出去。竖线要画对，只能包一层 wrapper，
   而那要改 DOM 结构、跟 React 打架，不值得。
   改用 opacity + 极淡底色：零布局影响，连续段落自然连成一片。 */
...
function buildSwitcherHTML(currentVariant) {
  ...
  <div class="dsh-bloom-menu" role="listbox" aria-label="Bloom 主题变体" hidden>${options}</div>
</div>`
}
