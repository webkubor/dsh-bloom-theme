...
function parseCmd(rawInput: string) {
  ...
  for (const tok of rest.split(/\s+/)) {
    if (!tok) continue
    if (tok === '--card') writeCard = true
    else if (tok.startsWith('--card=')) { writeCard = true; cardPath = tok.slice('--card='.length) }
    else dir = tok
  }
...
function apply(ctx: any) {
  ctx.commands.register({
    ...
    handler: (invocation: any) => {
      ...
      // 默认：纯文本摘要
      if (!writeCard) return { kind: 'success', text: renderStatsText(s) }
