...
## 2026-08-24 · 发了 6 个版本，本机一个都装不上（pnpm minimumReleaseAge）

### 现象

当天连发 0.8.0 → 0.8.5。本机 DSH 的 web profile 却始终装着旧版：
`dsh plugin up @kubor/dsh-bloom-theme@latest` 跑完，装上的是 **0.4.0**；
`pnpm update` 装 0.8.1；每次命令都成功退出，就是拿不到最新。

### 走过的弯路

先怀疑 registry 是 npmmirror 镜像、还没同步 —— 查完发现 registry 是**官方**
`registry.npmjs.org`，而且官方和镜像的 `dist-tags.latest` 都是 `0.8.5`。
`npm view` / `pnpm view` 三处查出来都对。也就是说：**元数据没问题，pnpm 就是不装它。**

### 根因

profile 的 `pnpm-workspace.yaml` 里躺着一行：
...
`minimumReleaseAge` 是 pnpm 11 默认开启的**供应链安全特性**：不安装发布时间不足
24 小时的版本（防的是刚被劫持发布的恶意版本）。对照发布时间就一目了然：
...
于是 pnpm **静默回退**到唯一"够老"的 0.4.0。而豁免名单里写的是带版本号的
`@0.3.4` —— 只豁免那一个版本，所以每发一次新版都会重新被挡。

这也顺带解释了另一件长期没人搞懂的事：profile 的依赖声明为什么一直停在
`^0.3.4` 再没更新过 —— 因为每次尝试升级都被这道墙挡回去了。

### 修法

去掉版本号，豁免整个包（作者就是本机用户，供应链风险为零）：
...
改完仍要注意 pnpm 的 **packument 缓存**：`up @latest` 拿到的是缓存里的旧
metadata，会装成 0.8.1。显式 `add @kubor/dsh-bloom-theme@0.8.5` 才一次到位。

`npm run preflight` 第 5 组现在会检查这条豁免：写成带版本号的形式直接判失败，
并提示去掉 `@x.y.z`（`link:` / `file:` 的本地声明不走 registry，自动跳过）。

### 教训

1. **「安全特性静默降级」是最难查的一类故障。** 命令成功、退出码 0、装上了一个
   *能用* 的旧版本 —— 没有任何一处报错说「你要的版本被策略挡了」。查这类问题要
   直接对照两边的事实（registry 有什么 / 本地装了什么），而不是顺着命令输出找。
2. **豁免、白名单、忽略项一律不要带版本号或具体值。** 带了就等于"只豁免这一次"，
   下次同样的场景重新踩。这跟本仓库范围白名单的思路正好相反 —— 白名单要精确
   （防外扩），豁免要宽泛（防复发），因为两者防的方向不同。
