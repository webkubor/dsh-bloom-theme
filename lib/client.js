...
function bindAmbiencePanel(panel) {
  ...
  panel.addEventListener('click', (e) => {
    ...
    if (wp) {
      ...
      update(m === 'auto' ? { mode: 'auto' } : m === 'custom' ? { mode: 'custom' } : { mode: 'fixed', fixed: wp.dataset.ambVariant })
      return
    }
  ...
  panel.addEventListener('change', (e) => {
    ...
    if (k === 'custom') {
      // data: URL 可能超 localStorage 配额（5MB）——超了就提示并回落预设
      try {
        window.localStorage.setItem(AMB_KEY, JSON.stringify({ ...readAmbience(), mode: 'custom', custom: el.value.trim() }))
        renderAmbience(readVariant())
      } catch {
        el.value = ''
        update({ mode: 'auto' })
