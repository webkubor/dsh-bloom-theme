...
export interface StatsResult {
  project: string           // 仓库/项目名（目录 basename）
  root: string              // git 根目录
  loc: number               // 有效代码行数（按已知语言计数）
  files: number             // 被 git 跟踪的文件数
  commits: number           // HEAD 提交数
  lastCommitted: string | null // 最近提交的相对时间
  activeToday: boolean      // 今天是否有提交
  streak: number            // 连续提交天数（含今天/昨天，取两者中最近的连续段）
  languages: Record<string, number> // 语言 -> 代码行数（降序）
  byExtension: number[]     // 与 languages 顺序对应的文件数
  locByExtension: number[]  // 与 languages 顺序对应的行数
}
...
function computeStreak(dates: string[]): number {
  ...
  const now = new Date()
  const today = day(now)
  const yest = day(new Date(now.getTime() - 86400_000))
  ...
  while (set.has(t.toISOString().slice(0, 10))) {
    streak++
    t = new Date(t.getTime() - 86400_000)
  }
  return streak
}

/** 读取一个目录的 git 统计。dir 非 git 仓库时向上找最近 git 根。 */
export function computeStats(dir: string): StatsResult {
  ...
  const fileByLang: Record<string, number> = {}
  let loc = 0
  ...
  for (const rel of tracked) {
    ...
    if (size === 0 || size > MAX_SINGLE_BYTES) continue
    let lines = 0
  ...
  let commits = 0
  try { commits = Number(git(['rev-list', '--count', 'HEAD'], root)) || 0 } catch { commits = 0 }

  let dates: string[] = []
  try { dates = git(['log', '--pretty=%ad', '--date=short'], root).split('\n').filter(Boolean) } catch { dates = [] }
  const today = new Date().toISOString().slice(0, 10)
  const streak = computeStreak(dates)
  const activeToday = dates.includes(today)

  let lastCommitted: string | null = null
  try { lastCommitted = git(['log', '-1', '--format=%cr'], root) || null } catch { lastCommitted = null }

  const langs = Object.keys(locByLang).sort((a, b) => locByLang[b] - locByLang[a])
  const languages: Record<string, number> = {}
  const byExtension: number[] = []
  const locByExtension: number[] = []
...
/** 一行可复制的纯文本摘要（命令输出用）。 */
export function renderStatsText(s: StatsResult): string {
...
export function renderStatsCardHTML(s: StatsResult): string {
  const total = s.loc || 1
  ...
  const numsHTML = nums.map(([v, l]) =>
    `<div class="num"><b>${v}</b><span>${l}</span></div>`).join('')
